...
public class ArenaCmd implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args.length==0||args[0].equalsIgnoreCase("help")) {
			...
			if(!(sender instanceof Player)) {
				sender.sendMessage("vous devez être un jouer pour pouvoir executer ceci");
				return false;
			}
			...
			try {
				...
				sender.sendMessage("§cCoordonénes invalides");
				return false;
			}
			main.arenas.add(new Arena(args[1], tpA,tpB));
			sender.sendMessage("l'arène a été créée");
			return true;
		}else if (args[0].equalsIgnoreCase("toggle")) {
			main.canStartDuel=!main.canStartDuel;
			if(main.canStartDuel) {
				sender.sendMessage("il est maintenant possible de créer des deuls");
				main.getServer().broadcastMessage("les duels on été réactivées");
				return true;
			}else {
				sender.sendMessage("il est maintetant impossible de créer des duels");
				main.getServer().broadcastMessage(sender.getName()+ " a temporairement désactivé les duels");
			...
			sender.sendMessage("l'arène "+arena.getName()+" à été supprimée");
			return true;
			...
			Arena arena= FonctionsArena.getArena(args[1]);
			if(arena==null) {
				sender.sendMessage("§cArène inconue");
				return false;
			}
			arena.setWorld(player.getWorld());
			player.sendMessage("le monde de "+arena.getName()+" à bien été modifié");
			return true;
			...
			sender.sendMessage("le point de téléportation à bien été modifié");
			return true;
			...
			main.saveArenas();
			sender.sendMessage("arènes sauvegardées");
			return true;
		}else if(args[0].equalsIgnoreCase("load")) {
			main.loadArenas();
			sender.sendMessage("arènes charchées");
			return true;
	...
	private String getHelpMessage() {
		...
		message+="/arena arenaList: pour afficher la liste des arène de duel\n";
		message+="/arena add <nom> <x1> <y1> <z1> <x2> <y2> <z2> : pour crée une nouvelle arène\n";
		message+="/arena toggle: pour activer/désactivé la possiblilité de créer de nouveaux duels\n";
		message+="/arena remove <arène>: pour supprimer une arène\n";
		message+="/arena setWorld <arène>: pour définir le monde de téléopration de l'arène au monde où le joueur se trouve\n";
		message+="/arena <setTpA||setTpB> <arène> <x> <y> <z>: pour définir un des endroit où le joueur est téléporté pour une arène\n";
		message+="/arena save: pour sauvegarder les arènes\n";
		message+="/arena load: pour chargées les arènes sauvegardées\n";
